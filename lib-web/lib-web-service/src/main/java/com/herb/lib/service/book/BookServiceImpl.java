...
public class BookServiceImpl implements BookService {
    ...
    public ResultDTO findListByName(String name) {
        ...
        if (CollectionUtils.isEmpty(list)){
            return new ResultDTO(HttpCode.FAIL.getCode(),"暂无相应的书籍");
        }
    ...
    public ResultDTO insert(BookDTO entity) {
        ...
        if(StringUtils.isEmpty(entity.getBookClassId())){
            return new ResultDTO(HttpCode.FAIL.getCode(),"书籍编号不能为空");
        }
    ...
    public ResultDTO findById(int id) {
        ...
        if (null == bookDTO){
            return new ResultDTO(HttpCode.FAIL.getCode(),"暂无对应的书籍数据");
        }

        logger.info("出参：" + bookDTO);
        return new ResultDTO(HttpCode.SUCCESS.getCode(),"寻找成功",bookDTO);
    }
}
